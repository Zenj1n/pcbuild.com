...
class CasesSpider(CrawlSpider):
   ...
   #)
    ...
    def parse_start_url(self,response):
        ...
        query = neo4j.CypherQuery(graph_db, "CREATE (inf_hdd {webshop:{webshop}, name:{name}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN inf_hdd")
                              
        inf_hdd = query.execute(webshop=webshop, name=name, url=url, desc=desc, price=price)