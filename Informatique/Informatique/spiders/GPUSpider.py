...
class CasesSpider(CrawlSpider):
   ...
   #)
    ...
    def parse_start_url(self,response):
        ...
        query = neo4j.CypherQuery(graph_db, "CREATE (inf_gpu {webshop:{webshop}, name:{name}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN n")
                              
        inf_gpu = query.execute(webshop=webshop, name=name, url=url, desc=desc, price=price)