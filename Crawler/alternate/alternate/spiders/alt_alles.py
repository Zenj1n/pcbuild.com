...
class alt_alles(CrawlSpider):
    ...
    def parse_start_url(self,response):
        ...
        query = neo4j.CypherQuery(graph_db, "CREATE (alt_alles {webshop:{webshop}, name:{name}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN alt_alles")
                              
        alt_alles = query.execute(webshop=webshop, name=name, url=url, desc=desc, price=price)
