...
class OpticalDriveSpider(CrawlSpider):
    ...
    def parse_start_url(self,response):
        ...
        query = neo4j.CypherQuery(graph_db, "CREATE (hw_opticalDriver {webshop:{webshop}, name:{name}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN hw_opticalDriver")
                              
        hw_opticalDriver = query.execute(webshop=webshop, name=name, url=url, desc=desc, price=price)