...
class MotherboardSpider(CrawlSpider):
    ...
    def parse_start_url(self,response):
        ...
        for titles in row:
           ...
           query = neo4j.CypherQuery(graph_db, "CREATE (hw_mb {webshop:{webshop}, name:{name}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN hw_mb")
                              
           hw_mb = query.execute(webshop=webshop, name=name, url=url, desc=desc, price=price)