...
class alt_ram_ddr(CrawlSpider):
    ...
    def parse_start_url(self,response):
        ...
        query = neo4j.CypherQuery(graph_db, "CREATE (alt_ram_ddr {webshop:{webshop}, name:{namedb}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN alt_ram_ddr")
                              
        alt_ram_ddr = query.execute(webshop=webshop, namedb=namedb, url=url, desc=desc, price=price)