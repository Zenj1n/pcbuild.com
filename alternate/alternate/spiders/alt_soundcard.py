...
class alt_soundcard(CrawlSpider):
    ...
    def parse_start_url(self,response):
        ...
        for titles in titles:
           ...
           query = neo4j.CypherQuery(graph_db, "CREATE (alt_soundcard {webshop:{webshop}, name:{namedb}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN alt_ram_ddr")
                              
           alt_soundcard = query.execute(webshop=webshop, namedb=namedb, url=url, desc=desc, price=price)