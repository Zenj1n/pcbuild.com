...
class alt_koel_nwcard(CrawlSpider):
    ...
    def parse_start_url(self,response):
        ...
        query = neo4j.CypherQuery(graph_db, "CREATE (alt_nwcard {webshop:{webshop}, name:{namedb}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN alt_nwcard")
                              
        alt_nwcard = query.execute(webshop=webshop, namedb=namedb, url=url, desc=desc, price=price)