...
class alt_drive(CrawlSpider):
    ...
    def parse_start_url(self,response):
        ...
        query = neo4j.CypherQuery(graph_db, "CREATE (alt_drive {webshop:{webshop}, name:{namedb}, url:{url}, desc:{desc}, price:{price}})"
                              "RETURN alt_drive")
                              
        alt_drive = query.execute(webshop=webshop, namedb=namedb, url=url, desc=desc, price=price)